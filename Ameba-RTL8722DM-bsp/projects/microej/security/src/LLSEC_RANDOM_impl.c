...
int32_t LLSEC_RANDOM_IMPL_init(void)
{
    ...
    LLSEC_RANDOM_DEBUG_TRACE("%s: L%4d \n", __func__,__LINE__);
    ...
    if (!initialized) {
    	...
    	mbedtls_entropy_add_source(&entropy, llsec_customer_entropy_source, NULL,
    								   MBEDTLS_ENTROPY_MAX_GATHER,
    								   MBEDTLS_ENTROPY_SOURCE_STRONG);
        return_code = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy,
                                          (const uint8_t*)pers, 0);
        LLSEC_RANDOM_DEBUG_TRACE("%s: L%4d,[rc]:%d \n", __func__,__LINE__,return_code);
...
int llsec_customer_entropy_source(void *data, uint8_t *output, size_t len, size_t *olen)
{
    uint32_t seed;
